...
RSpec.describe(JsonModel::Types::Primitive::String) do
  describe('#as_schema') do
    ...
    it('includes the minLength attribute') do
      expect(described_class.min_length(10).as_schema)
        .to(eq({ type: 'string', minLength: 10 }))
    end

    it('includes the maxLength attribute') do
      expect(described_class.max_length(10).as_schema)
        .to(eq({ type: 'string', maxLength: 10 }))
    end

    it('includes the pattern attribute') do
      expect(described_class.pattern(/\A\w+\z/).as_schema)

...
RSpec.describe(JsonModel::Types::Primitive::Numeric) do
  describe('#as_schema') do
    it('includes the multipleOf attribute') do
      expect(described_class.new(schema_type: 'number', types: [Float, Integer]).multiple_of(5).as_schema)
        .to(eq({ type: 'number', multipleOf: 5 }))
    end

    it('includes the minimum attribute') do
      expect(described_class.new(schema_type: 'number', types: [Float, Integer]).minimum(5).as_schema)
        .to(eq({ type: 'number', minimum: 5 }))
    end

    it('includes the maximum attribute') do
      expect(described_class.new(schema_type: 'number', types: [Float, Integer]).maximum(5).as_schema)
        .to(eq({ type: 'number', maximum: 5 }))
    end

    it('includes the exclusiveMinimum attribute') do
      expect(described_class.new(schema_type: 'number', types: [Float, Integer]).exclusive_minimum(5).as_schema)
        .to(eq({ type: 'number', exclusiveMinimum: 5 }))
    end

    it('includes the exclusiveMaximum attribute') do
      expect(described_class.new(schema_type: 'number', types: [Float, Integer]).exclusive_maximum(5).as_schema)
  ...
  describe('.register_validations') do
    ...
    context('for multiple of') do
      before do
        described_class
          .new(schema_type: 'number', types: [Float, Integer])
          .multiple_of(5)

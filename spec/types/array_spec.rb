...
RSpec.describe(JsonModel::Types::Array) do
  describe('#as_schema') do
    it('returns an array schema of primitive types') do
      expect(
        described_class.new(JsonModel::Types.string).as_schema,
      )
        .to(
          eq(
            {
    ...
    it('returns an array schema of nested array types') do
      expect(
        described_class.new(described_class.new(JsonModel::Types.string)).as_schema,
      )
        .to(
          eq(
            {
    ...
    it('includes the minItems attribute') do
      expect(
        described_class.new(JsonModel::Types.string).min_items(10).as_schema,
      )
        .to(
          eq(
            {
    ...
    it('includes the maxItems attribute') do
      expect(
        described_class.new(JsonModel::Types.string).max_items(10).as_schema,
      )
        .to(
          eq(
            {

...
RSpec.describe(JsonModel::Configuration) do
  describe('#property_naming_strategy') do
    it('defaults to identity') do
      value = SecureRandom.uuid.to_sym
      expect(JsonModel.config.property_naming_strategy.call(value))
        .to(eq(value))
    end
    ...
    it('can be changed to pascal case') do
      ...
      expect(JsonModel.config.property_naming_strategy.call(:foo_bar))
        .to(eq('FooBar'))
    end
